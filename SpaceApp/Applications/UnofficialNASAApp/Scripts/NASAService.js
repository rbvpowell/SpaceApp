...
class NASAService {
    constructor() {
        ...
        this.apodClient = new NASAAPODClient_1.NASAAPODClient(url, apiKey);
        this.neoWClient = new NASANeoWClient_1.NASANeoWClient(url, apiKey);
    }
    getAPODAsync(date) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                var apod = yield this.apodClient.getAPODAsync(date, true);
            }
            catch (er) {
                date.setDate(date.getDate() - 1);
                apod = yield this.apodClient.getAPODAsync(date, true);
            }
    ...
    getNeosAsync() {
        return __awaiter(this, void 0, void 0, function* () {
            let date = new Date();
            var neos = yield this.neoWClient.getNeoFeedAsync(date, date);
            return neos.result;
        });
    }
    generateCardPromise(date) {
        return new Promise((resolve, reject) => {
            this.generateCard(date, resolve, reject);
        });
    }
    generateCard(date, resolve, reject) {
    ...
    updateFilter() {
        var selectField = this.fieldSelect[0];
        var selectType = this.typeSelect[0];
        var inputValue = this.valueInput[0];
    ...
    generateNeoTable() {
        ...
        neosPromise.then((neos) => {
            $(document).ready(function () {
                ...
                self.fieldSelect.append(emptyOption);
                var nameOption = $("<option value=\"name\">Name</option>");
                self.fieldSelect.append(nameOption);
                var urlOption = $("<option value=\"nasa_jpl_url\">URL</option>");
                self.fieldSelect.append(urlOption);
                var magnitudeOption = $("<option value=\"absolute_magnitude_h\">Magnitude</option>");
                self.fieldSelect.append(magnitudeOption);
                var hazardousOption = $("<option value=\"is_potentially_hazardous_asteroid\">Potentially Hazardous</option>");
                self.fieldSelect.append(hazardousOption);
                var distanceOption = $("<option value=\"miss_distance\">Miss distance</option>");
                self.fieldSelect.append(distanceOption);
                var orbitingOption = $("<option value=\"orbiting_body\">Orbiting body</option>");
                self.fieldSelect.append(orbitingOption);
                var velocityOption = $("<option value=\"relative_valocity\">Velocity</option>");
                self.fieldSelect.append(velocityOption);
                var collisionOption = $("<option value=\"is_sentry_object\">Collision predicted</option>");
                self.fieldSelect.append(collisionOption);
                ...
                var likeOption = $("<option value=\"like\">like</option>");
                self.typeSelect.append(likeOption);
                var equalsOption = $("<option value=\"=\">=</option>");
                self.typeSelect.append(equalsOption);
                var lessThanOption = $("<option value=\"<\"><</option>");
                self.typeSelect.append(lessThanOption);
                var lessThanOrEqualOption = $("<option value=\"<=\"><=</option>");
                self.typeSelect.append(lessThanOrEqualOption);
                var greaterThanOption = $("<option value=\">\">></option>");
                self.typeSelect.append(greaterThanOption);
                var greaterThanOrEqualOption = $("<option value=\">=\">>=</option>");
                self.typeSelect.append(greaterThanOrEqualOption);
                var notEqualOption = $("<option value=\"!=\">!=</option>");
                self.typeSelect.append(notEqualOption);
...
$(document).ready(function () {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (var a = 1; a < 22; a++) {
            ...
            yield nasaService.generateCardPromise(date);
            count++;
        }
